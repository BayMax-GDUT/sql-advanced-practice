...
select sc1.SNO, sc1.CNO, sc2.CNO, sc1.SCORE
from sc sc1, sc sc2
where sc1.SNO = sc2.SNO and sc1.CNO <> sc2.CNO and sc1.SCORE = sc2.SCORE

#42、查询每门功成绩最好的前两名

select sc.SNO, sc.CNO, sc.SCORE
from sc
where (select count(1) + 1 from sc sc1 where sc.CNO = sc1.CNO and sc1.SCORE > sc.SCORE) < 3
...
select student.*, sc1.SCORE, sc2.SCORE
from student, sc1, sc2
where student.SNO = sc1.SNO and student.SNO = sc2.SNO
and sc1.CNO = 01 and sc2.CNO = 02 and sc1.SCORE > sc2.SCORE
...
select student.SNO, sc.CNO, sc.SCORE
from student, sc
where student.SNO = sc.SNO
```
